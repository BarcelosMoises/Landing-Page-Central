...
export function NavPrimaria() {
  ...
  function NavItemMobile({ id, label, href }: { id: string; label: string; href: string }) {
    ...
    return (
      <Link href={href} onClick={fecharMenu} className={baseClasses} style={isAtiva ? { backgroundColor: "color-mix(in srgb, var(--color-service-accent, #800000) 20%, transparent)" } : undefined}>
        {label}
      </Link>
    );
  }
            ...
            <nav aria-label="Menu mobile" className="mx-auto flex max-w-7xl flex-col gap-1 px-4 pb-5 pt-3">
              <motion.div initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -8 }} transition={{ duration: 0.2 }}>
                <button
                  ...
                  className="flex w-full items-center justify-between rounded-lg px-4 py-3 text-left text-sm font-medium text-neutral-300 transition-colors duration-200 hover:bg-white/10 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#800000]"
                >
                  Serviços
                ...
                <AnimatePresence initial={false}>
                  {servicosAberto && (
                    ...
                    >
                      <ul className="ml-4 border-l border-white/15 py-1">
                        {SERVICOS_MENU.map((servico, index) => (
                          <motion.li key={servico.href} initial={{ opacity: 0, x: -6 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -6 }} transition={{ delay: 0.03 * index, duration: 0.18 }}>
                            <Link href={servico.href} onClick={fecharMenu} className="block rounded-md px-4 py-2 text-sm text-white/80 transition-colors duration-150 hover:bg-white/10 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#800000]">
                              {servico.label}
                            </Link>
                          </motion.li>
                        ))}
