...
| 36 | 🟦 | `components/CtaFinal.tsx` | Fundo da seção em `bg-[#1a0000]` + `<CrosshairDecor />`. **Parcial (Ago 2026):** o fundo correto e a retícula entraram via `CtaFinal` + adoção nas 7 rotas. **Pendente:** limpar os comentários desatualizados de `CrosshairDecor.tsx` (linhas 4 e 46 citam `bg-neutral-900`) | DESIGN.md → Hierarquia dos Fundos Escuros | 🔴 |
| 37 | 🟦 | `components/CtaFinal.tsx` | Tokens de texto na CTA. **Parcial (Ago 2026):** heading `text-white`, subtítulo `#c4a8a8` e ghost `mailto:` em `#e0c8c8` entraram via `CtaFinal` — os subtítulos `text-neutral-400` das seções inline foram removidos com elas. **Pendente:** verificar o contexto dos 5 ícones SVG com `text-neutral-400` (avcb:186, spda:167, vigilancia:175, ambiental:181, prefeitura:201) — manter apenas se estiverem sobre fundo claro | DESIGN.md → Tokens sobre `#1a0000` | 🔴 |
...
> - `text-neutral-400` sobre fundo escuro ×~15 — subtítulos da CTA (**resolvidos** via CtaFinal), labels "Base Normativa" (task #39) e 1 item de estado no card do AVCB (task #37); demais `neutral-400`/`neutral-900` estão sobre fundo claro (permitido)
